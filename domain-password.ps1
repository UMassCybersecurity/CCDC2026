...
foreach($user in $users)
{
    ...
    # Skip prompting if user already selected No to All
    if ($NoToAll) {
        Write-Output "Skipping (No to All): $($user.SamAccountName)"
        continue
    }

    # Prompt user unless Yes to All was selected
    if (-not $YesToAll) {

        Write-Host ""
        Write-Host "Change password for user: $($user.SamAccountName) ?" -ForegroundColor Cyan
        $response = Read-Host "[Y] Yes  [N] No  [A] Yes to All  [X] No to All"

        switch ($response.ToUpper()) {
            "Y" { }  # continue processing
            "N" { Write-Output "Skipping user: $($user.SamAccountName)"; continue }
            "A" { $YesToAll = $true }
            "X" { $NoToAll = $true; Write-Output "Skipping all remaining users..."; continue }
            default { Write-Output "Invalid response, skipping user."; continue }
        }
    }
