...
if ! pw usershow "$syslog_proxy_user" >/dev/null 2>&1; then
    pw useradd "$syslog_proxy_user" -m -s /bin/sh
fi

# Create the Python script
cat > /tmp/syslog_proxy.py << 'EOF'
...
# See the files at /usr/local/etc/rc.d/syslog_proxy_**** for an example of how to manage FreeBSD services for them.
...
mv /tmp/syslog_proxy.py "$syslog_proxy_script"
chmod 755 "$syslog_proxy_script"
chown "$syslog_proxy_user:$syslog_proxy_user" "$syslog_proxy_script"

# Create the FreeBSD rc.d script
cat > /tmp/syslog_proxy_15140 << 'EOF'
#!/bin/sh

# PROVIDE: syslog_proxy_15140
# REQUIRE: NETWORKING
# KEYWORD: shutdown

. /etc/rc.subr

name="syslog_proxy_15140"
rcvar="syslog_proxy_15140_enable"

pidfile="/var/run/${name}.pid"
command="/usr/sbin/daemon"
command_args="-f -p ${pidfile} /usr/local/bin/python3 /usr/local/bin/syslog_proxy.py --listen-port 15410 --forward-host 127.0.0.1 --forward-port 5140"

load_rc_config $name
run_rc_command "$1"
EOF

# Move the rc.d script and set permissions
mv /tmp/syslog_proxy_15140 "/usr/local/etc/rc.d/syslog_proxy_15140"
chmod 755 "/usr/local/etc/rc.d/syslog_proxy_15140"

# Add the service to rc.conf to enable it on boot
if ! grep -q "syslog_proxy_15140_enable" /etc/rc.conf; then
    echo 'syslog_proxy_15140_enable="YES"' >> /etc/rc.conf
fi

# Start the service
service syslog_proxy_15140 start

echo "Syslog proxy installation complete!"
echo "Service status:"
service syslog_proxy_15140 status
